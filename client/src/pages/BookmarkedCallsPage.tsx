...
import { useBookmarkedCallsFilterStore } from "@/store/filter-store";
...
export default function BookmarkedCallsPage() {
  const navigate = useNavigate();
  const { filters, page, setFilters, setPage } = useBookmarkedCallsFilterStore();
  ...
  useEffect(() => {
    ...
    qc.prefetchQuery({ queryKey: ["bookmarked-calls", nextParams], queryFn: () => apiGetAllCalls(nextParams), staleTime: STALE_MS });
  }, [queryParams]);
    ...
    <div className="flex flex-col gap-8 animate-in fade-in duration-500">
      ...
      <div>
        <div className="flex items-center gap-2.5 mb-1">
          <Bookmark className="h-5 w-5" style={{ color: "#f7f8f8" }} fill="#f7f8f8" />
          <h1
            className="text-3xl"
        ...
        <p className="mt-1.5 text-sm" style={{ color: "#8a8f98", fontWeight: 400, fontFeatureSettings: FF }}>
          Curated calls flagged for golden dataset review.
        </p>
      </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            >
              {hasFilters
                ...
                : <Bookmark className="h-5 w-5" style={{ color: "rgba(255,255,255,0.15)" }} />
              }
            </div>
            <p
            ...
            >
              {hasFilters ? "No Bookmarked Calls Match Filters" : "No Bookmarked Calls Yet"}
            </p>
            <p
            ...
            >
              {hasFilters
                ? "Adjust filters to expand results"
                : "Bookmark calls from /call/:id or /eval/:id"}
            </p>
            {hasFilters && (
              <button
...
function BookmarkCallRow({ call, onView }: { call: RawCall; onView: () => void }) {
  ...
  return (
    ...
    >
      <td
        ...
        style={{ color: "#7170ff", fontFamily: "Berkeley Mono, ui-monospace, SF Mono, Menlo, monospace" }}
      >
        #{call.call_id}
      </td>
      <td className="px-4 py-3">
        <div className="flex flex-col gap-0.5">
          <span className="text-sm" style={{ color: "#d0d6e0", fontWeight: 510, fontFeatureSettings: FF }}>
            {call.organization_name || "—"}
          </span>
          {call.rooftop_name && (
            <span className="text-[10px]" style={{ color: "#62666d", fontFeatureSettings: FF }}>
              {call.rooftop_name}
            </span>
          )}
        </div>
      </td>
      <td className="px-4 py-3 text-xs" style={{ color: "#8a8f98", fontFeatureSettings: FF }}>
        {call.campaign || "—"}
      </td>
      <td className="px-4 py-3 text-xs" style={{ color: "#62666d", fontFeatureSettings: FF }}>
      ...
      <td className="px-4 py-3">
        {call.direction?.toLowerCase() === "inbound"
          ? <span className="flex items-center gap-1 text-[10px] uppercase tracking-widest" style={{ color: "#10b981" }}><ArrowDownLeft className="h-3 w-3" />In</span>
          : call.direction?.toLowerCase() === "outbound"
            ? <span className="flex items-center gap-1 text-[10px] uppercase tracking-widest" style={{ color: "#7170ff" }}><ArrowUpRight className="h-3 w-3" />Out</span>
      ...
      <td className="px-4 py-3">
        <button
          ...
          style={{
            background: "rgba(255,255,255,0.02)", color: "#62666d",
            borderColor: "rgba(255,255,255,0.08)", fontWeight: 510, fontFeatureSettings: FF,
          }}
          onMouseEnter={(e) => {
...
function CallStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className="px-2 py-0.5 rounded-full text-[10px] uppercase tracking-widest border"
      style={{ background: "rgba(255,113,108,0.1)", color: "#ff716c", borderColor: "rgba(255,113,108,0.2)", fontWeight: 510, fontFeatureSettings: FF }}
    >
      {status || "Unknown"}
    </span>
  );
